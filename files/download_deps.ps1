...
if (-not (Test-Path -LiteralPath $gitFile) -or (Get-Item -LiteralPath $gitFile).Length -lt 1000000) {
...
if (-not (Test-Path -LiteralPath $nodeFile) -or (Get-Item -LiteralPath $nodeFile).Length -lt 1000000) {
...
if (-not (Test-Path -LiteralPath $pnpmFile) -or (Get-Item -LiteralPath $pnpmFile).Length -lt 1000000) {
...
Get-ChildItem -LiteralPath $depsDir | Select-Object Name, @{Name="Size (MB)"; Expression={[math]::Round($_.Length / 1MB, 2)}} | Format-Table -AutoSize
